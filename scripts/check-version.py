...
Usage:
    python3 scripts/check-version.py <tag>
...
def check_pyproject_toml(expected: str) -> CheckResult | None:
    path = Path("pyproject.toml")
    if not path.exists():
        return None

    if tomllib is None:
        # Fall back to regex if tomllib isn't available
...
def main() -> int:
    ...
    tag = sys.argv[1]
    version = tag.removeprefix("v")  # normalise: v1.2.3 → 1.2.3
